...
').map(u => u.trim()).filter(Boolean);
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      try {
        ...
        if (facebookProfilePic) {
          ...
          const validLogos = brandData.logos.filter((l: any) => isValidLogoUrl(l.url));
          const transparentLogo = validLogos.find((l: any) => l.type === 'icon' && (l.mode === 'light' || l.mode === 'dark'));
          const iconLogo = transparentLogo || validLogos.find((l: any) => l.type === 'icon') || validLogos[0];
        ...
        if (selectedLogo?.colors?.length > 0) {
          ...
          if (logoMode === 'dark') {
            iconBgColor = brandData.colors[brandData.colors.length - 1]?.hex || '#FFFFFF';
          } else if (logoMode === 'has_opaque_background') {
            iconBgColor = '#FFFFFF';
          } else {
            iconBgColor = brandData.colors[0]?.hex || '#FFFFFF';
          }
        }
  ...
  const applyBulkEdits = () => {
    ...
    const updatedCards = generatedCards.map(card => ({ ...card, title: newTitle, body: newBody, messageType: editingTemplate }));
    setGeneratedCards(updatedCards);
  };
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          a.href = url;
          a.download = card.brandName.replace(/s+/g, '-') + '-notification.png';
          a.click();
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-8">
          <button onClick={() => setMode('single')} className={'px-6 py-2 rounded-lg font-medium transition-colors ' + (mode === 'single' ? 'bg-purple-600 text-white' : 'bg-[#1a1a1a] text-gray-400 hover:text-white')}>
            Single Mode
          </button>
          <button onClick={() => setMode('bulk')} className={'px-6 py-2 rounded-lg font-medium transition-colors ' + (mode === 'bulk' ? 'bg-purple-600 text-white' : 'bg-[#1a1a1a] text-gray-400 hover:text-white')}>
            Bulk Mode
          </button>
        </div>
          ...
          <div className="bg-[#1a1a1a] rounded-xl p-8 border border-gray-800">
            ...
            <div className="mb-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={skipCache} onChange={(e) => setSkipCache(e.target.checked)} className="w-4 h-4 bg-[#0f0f0f] border border-gray-800 rounded focus:outline-none focus:ring-2 focus:ring-purple-500" />
            ...
            {mode === 'single' ? (
              ...
              <div className="space-y-4">
                <label className="block">
                  ...
                  <textarea value={bulkUrls} onChange={(e) => setBulkUrls(e.target.value)} rows={8} className="w-full px-4 py-3 bg-[#0f0f0f] border border-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-white font-mono text-sm" />
                </label>
                <button onClick={generateCards} disabled={isGenerating} className="px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 rounded-lg font-medium transition-colors flex items-center gap-2">
                  {isGenerating && <Loader2 className="w-4 h-4 animate-spin" />}
            ...
            {error && (
              <div className="mt-4 p-4 bg-red-500/10 border border-red-500/50 rounded-lg text-red-400">{error}</div>
            )}
          </div>
        ) : (
            ...
            <div className="space-y-6">
              <div className="bg-[#1a1a1a] rounded-xl p-6 border border-gray-800">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {generatedCards.length > 1 && (
                      <>
                        <button onClick={() => setCurrentCardIndex(Math.max(0, currentCardIndex - 1))} disabled={currentCardIndex === 0} className="p-2 bg-[#0f0f0f] hover:bg-gray-800 disabled:opacity-50 rounded-lg transition-colors">
                          <ChevronLeft className="w-5 h-5" />
                        </button>
                        <span className="text-sm text-gray-400">{currentCardIndex + 1} / {generatedCards.length}</span>
                        <button onClick={() => setCurrentCardIndex(Math.min(generatedCards.length - 1, currentCardIndex + 1))} disabled={currentCardIndex === generatedCards.length - 1} className="p-2 bg-[#0f0f0f] hover:bg-gray-800 disabled:opacity-50 rounded-lg transition-colors">
                ...
                <div className="flex justify-center">
                  {currentCard && <NotificationCard ref={cardRef} data={currentCard} />}
                </div>
                <div className="flex gap-2 mt-6">
                  <button onClick={() => exportToPng()} className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
                    <Download className="w-4 h-4" />
                    Export PNG
                  </button>
                  {mode === 'bulk' && generatedCards.length > 1 && (
                    <button onClick={exportAllToPng} className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
                      <Download className="w-4 h-4" />
                      Export All
                    </button>
                  )}
              ...
              <button onClick={() => setGeneratedCards([])} className="w-full px-4 py-2 bg-[#1a1a1a] hover:bg-gray-800 border border-gray-800 rounded-lg font-medium transition-colors">
                Start New
              </button>
            </div>
            <div className="space-y-6">
              <div className="bg-[#1a1a1a] rounded-xl p-6 border border-gray-800">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={mode === 'bulk' ? applyBulkEdits : applyManualEdits} className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg font-medium transition-colors text-sm">
                      {mode === 'bulk' ? 'Apply to All' : 'Apply Changes'}
                    </button>
                    <button onClick={() => generateCards()} disabled={isGenerating} className="flex-1 px-4 py-2 bg-[#0f0f0f] hover:bg-gray-900 border border-gray-800 rounded-lg font-medium transition-colors text-sm flex items-center justify-center gap-2">
